...
class DatasetWrapper(VisionDataset):
    allowed_keywords = ["idx2name", "name2idx", "target_classes"]
    ...
    def load_dataset(self, dataset_name, samples_per_class, random_seed: int = 42):
        ...
        if dataset_name == DS_IMAGENET:
            ...
            dataset = NABirdsDataset(NABIRDS_DIR, train=False, transform=self.transform, return_path=False)
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_name in [DS_IMAGENET, DS_IMAGENET_A, DS_IMAGENET_C, DS_PLACES365, DS_CUB, DS_NABIRDS]:
