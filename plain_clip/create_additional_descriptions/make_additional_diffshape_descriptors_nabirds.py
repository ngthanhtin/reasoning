...
def remove_annotation(name):
    ...
    return name, ''
# %%
additional_chatgpt_desc_path = "/home/tin/reproduce_visual_descriptors/classify_by_description_release/descriptors/additional_chatgpt_descriptors_nabirds.json"

# %% -----CHATGPT-----
f = open(additional_chatgpt_desc_path, 'r')
additional_chatgpt_desc_data = json.load(f)

# %%
num_birds_has_ann = 0
for bird in additional_chatgpt_desc_data:
    if '(' in bird:
        num_birds_has_ann+=1
num_birds_has_ann
    ...
    if allaboutbird_name != '':
        ...
        for ann in allaboutbird_birdtype_data[allaboutbird_name]:
            ...
            if compute_similarity(ann, ann_in_chatgpt_name) <= 1 and compute_similarity(ann, ann_in_chatgpt_name) >= -1:
                print(additional_chatgpt_desc_data[chatgpt_name][12])
                additional_chatgpt_desc_data[chatgpt_name][12] = allaboutbird_birdtype_data[allaboutbird_name][ann]
                print(additional_chatgpt_desc_data[chatgpt_name][12])
                print('------')
                num_changes += 1
...
changed_birds
# %%
additional_chatgpt_desc_data
# %%
# Serializing json
json_object = json.dumps(additional_chatgpt_desc_data, indent=4)
with open("additional_diffshape_chatgpt_descriptors_nabirds.json", "w") as outfile:
