...
for dataset in "${datasets[@]}"; do
    for model_size in "${model_sizes[@]}"; do
        python ../main.py --dataset="$dataset" --mode="gpt_descriptions" --sci2comm --model_size="$model_size" --descriptor_fnames "${descriptor_paths[@]}" --device cuda:1 --savename="normal_sci2comm_inat_accuracy"
    done
    
    for model_size in "${model_sizes[@]}"; do
        python ../main.py --dataset="$dataset" --mode="waffle" --sci2comm --model_size="$model_size" --descriptor_fnames "${descriptor_paths[@]}" --device cuda:1 --savename="waffle_sci2comm_inat_accuracy" --seeds "${seeds[@]}"
    done
done
