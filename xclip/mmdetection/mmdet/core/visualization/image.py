    ...
    for i, (pos, label) in enumerate(zip(positions, labels)):
        ...
        if scores is not None:
            if len(scores.shape) == 1:
                ...
                score_in_text = "|".join([f'{score:.02f}' for score in scores[i]])
                label_text += f'|{score_in_text}'
...
def imshow_det_bboxes(img,
    ...
    if bboxes is not None:
        ...
        scores = bboxes[:, 4] if bboxes.shape[1] == 5 else scores
        draw_labels(
            ax,
            labels[:num_bboxes],
            positions,
